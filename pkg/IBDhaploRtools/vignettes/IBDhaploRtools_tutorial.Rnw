...
## phased.gold <- ibdhap.make.states( qibd.filename = my.qibd.filename, 
...
transitions to state 15 with a one-marker no-call in between.
...
\item [ibdhap.names:] This function reads the identifying information
  ...
  \verb!ibdhap.make.states!, the ids file can be specified by either
...
You may wish to read in raw output from fgl2ibd which is a
space-delimited file the first two columns of which are the
identification number of the individuals in the pair, followed by the
ibd state for each marker, with a row for each pair. A file called
\verb|'outfifteenibd.txt'| could be converted to the format expected
by these functions which is a column per pair with no identification
numbers using, for example,
\begin{verbatim}
## read the whole table into R 
temp <- read.table('outfifteenibd_Gold.txt', sep = " ", header = FALSE )

## remove identifiers and transpose 
trueibd_phased <- t( temp[ ,-c(1:2)])
\end{verbatim}
...
corr.bylen.phased <-ibdhap.correct.bylen( phased.gold, 
                     ...
                     ibd.only=FALSE, position=posvec)
