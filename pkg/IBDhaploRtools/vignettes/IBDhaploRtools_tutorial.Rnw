...
\item[ibdhap.make.true:] This function reads in the true ibd states
  for each locus and each pair of chromosomes. This information will
  be used later for assessing the accuracy of the calls.
  
  The true IBD state labelling is assumed to be the Jaquard ordering.

  You may wish to read in raw output from fgl2ibd which is a
  space-delimited file the first two columns of which are the
  identification number of the individuals in the pair, followed by
  the ibd state for each marker, with a row for each pair. This file
  is usually named outnineibd.txt or outfifteenibd.txt. Use this function
  with the file location. For example,
