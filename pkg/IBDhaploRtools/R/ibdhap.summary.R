ibdhap.summary <-
function( states.dat, data.type = c("h", "g","r"), position=NA){
...
mean.any.ibd<-sum((dat<no.ibd.ind)*(dat>0))/((nrow(dat))*ncol(dat))
mean.no.call<-sum((dat==0))/((nrow(dat))*ncol(dat))
...
ibd.states<- NULL
   ...
   temp<-ibdhap.seg.lengths( dat[,icol], position = position )
