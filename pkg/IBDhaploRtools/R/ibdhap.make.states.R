...
ibdhap.make.states <-function( qibd.filename, dat.filename, cutoff = .8){
  ...
  par.file <- read.table(dat.filename)

  ## information form qibd file
  rawdat <- read.table(qibd.filename)
  print("Data read in successfully")

  ## information that I will use
  n.sets <- dim(par.file)[1] ##num sets is num of rows in file
  n.snps <- dim(rawdat)[1]/n.sets ## num of snps (assuming its the same in all the sets)
  n.col <- dim(rawdat)[2] ## number of columns in rawdat matrix
  ...
  ## empty matrix that will eventually be the output
  ibd.states <- matrix(NA, ncol=n.sets, nrow=n.snps)
    
  for( iset in 1:n.sets){
    
    ## subset just the data for the markers in the set.
    ind <- which( rawdat[,1]==iset)
    ibd.dat <- t(rawdat[ind, 2:n.col])

    ## truncate based on "cutoff" : put 1 if cutoff is met, 0 otherwise
    ibd.trunc <- apply(ibd.dat[ 3:(n.col-1), ], c(1,2), meet.cutoff, cutoff)
